...
import com.atin.arcface.widget.FocusView;
...
public class FaceHelper implements FaceListener {
    ...
    private int distancePixel = Constants.MIN_FACE_SIZE_RGB;
    ...
    private FaceHelper(Builder builder) {
        ...
        distancePixel = pref.getInt(Constants.PREF_DISTANCE_DETECT_PIXEL, Constants.MIN_FACE_SIZE_RGB);
    }
    ...
    public List<FacePreviewInfo> onPreviewFrame(byte[] nv21) {
        if (faceListener != null) {
            ...
            if (ftEngine != null) {
                //Kiểm tra khẩu trang và chất lượng ảnh
                int code = ftEngine.process(nv21, previewSize.width, previewSize.height, FaceEngine.CP_PAF_NV21, faceInfoList, FaceEngine.ASF_MASK_DETECT);
                if (code == ErrorInfo.MOK) {
                ...
                for (int i = 0; i < faceInfoList.size(); i++) {
                    ...
                    if (!maskInfoList.isEmpty()) {
                        ...
                        if (maskInfo.getMask() == MaskInfo.UNKNOWN) {
                            facePreviewInfoList.clear();
                            return facePreviewInfoList;
                        }
                    ...
                    int isMask = facePreviewInfo.getMask();
                    float destQuality = 0;
                    float quality = 0;

                    ImageQualitySimilar imageQualitySimilar = new ImageQualitySimilar();
                    int qualityCode = ftEngine.imageQualityDetect(nv21, previewSize.width, previewSize.height, FaceEngine.CP_PAF_NV21, faceInfoList.get(0),
                            facePreviewInfo.getMask(), imageQualitySimilar);
                    if (qualityCode == ErrorInfo.MOK) {
                        if (isMask == MaskInfo.WORN) {
                            destQuality = ConfigUtil.getImageQualityMaskRecognizeThreshold(mContext);
                        } else {
                            destQuality = ConfigUtil.getImageQualityNoMaskRecognizeThreshold(mContext);
                        }

                        //Nếu ảnh không đạt chất lượng tối thiểu thì bỏ qua frame đó
                        quality = imageQualitySimilar.getScore();
                    ...
                    //Check postion
//                    int x = (faceInfoList.get(i).getRect().right + faceInfoList.get(i).getRect().left)/2;
//                    int y = (faceInfoList.get(i).getRect().bottom + faceInfoList.get(i).getRect().top)/2;
//
//                    int bufferHeigh = 480/3;
//                    int bufferWidth = 640/3;
//
//                    if(x < bufferWidth || x > 640 - bufferWidth){
//                        facePreviewInfoList.clear();
//                        return facePreviewInfoList;
//                    }
//
//                    if(y < bufferHeigh || y > 480 - bufferHeigh){
//                        facePreviewInfoList.clear();
//                        return facePreviewInfoList;
//                    }
                    //End postion
                    ...
                    if( width > distancePixel && heigh > distancePixel)
                    {
                        facePreviewInfoList.add(facePreviewInfo);
                    }

                    //Check vị trí khuôn mặt so với dấu chấm đỏ trên màn hinh dựa vào vùng trán
                    if(faceInfoList.get(i).getForeheadRect().centerX() < 350 || faceInfoList.get(i).getForeheadRect().centerX() > 650
                            || faceInfoList.get(i).getForeheadRect().centerY() < 200 || faceInfoList.get(i).getForeheadRect().centerY() > 550){
                        facePreviewInfoList.clear();
                        return facePreviewInfoList;
                    }
                }
